...
export function toMentorMenteeDetailsDto(user: UserDocument): MentorMenteeDetailsDto {
  if (!user) throw new Error('User document not found for mapping.');

  const obj = user.toObject({ getters: true });
  const dto = new MentorMenteeDetailsDto();
  ...
  const interest = obj.interestId as any;
  dto.profileInfo = interest?.profileInfo || '';
  dto.churchDetails = interest?.churchDetails || [];
  dto.conference = interest?.conference || '';
