...
export class RoadMapsService {
  ...
  async create(dto: CreateRoadMapDto): Promise<RoadMapResponseDto> {
    ...
    if (existing) {
      throw new BadRequestException(
        `RoadMap with name '${dto.name}' already exists.`,
      );
    }
  ...
  async getCommentThread(
    ...
    const thread = await this.commentsModel
      .findOne({
    ...
    if (!thread) {
      throw new NotFoundException(
        `Comment thread for user ${userId} on roadmap ${roadMapId} not found`,
      );
    }
  ...
  async addComment(
    ...
    const updatedThread = await this.commentsModel
      .findOneAndUpdate(
    ...
    const pipeline: any[] = [
...

      {
        $lookup: {
          from: 'users',
          let: {
            mentorId: '$queries.repliedMentorId',
            queryStatus: '$queries.status',
          },
          pipeline: [
            {
              $match: {
  ...
  async replyQuery(
    ...
    if (!updatedThread) {
      throw new NotFoundException(
        `Query thread or item ID ${queryItemId} not found.`,
      );
    }
